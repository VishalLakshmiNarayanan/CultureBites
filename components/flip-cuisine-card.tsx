      ...
      <div className={`flip-card-container ${isFlipped ? "flipped" : ""}`}>
        <div className="flip-card" onClick={() => setIsFlipped(!isFlipped)}>
          ...
          <div className="card-back">
            <div className="w-full h-full p-6 flex flex-col overflow-y-auto text-gray-800">
                ...
                {host && (
                  <div className="flex flex-col items-center">
                    {host.profileImage ? (
                      ...
                      <div className="w-20 h-20 rounded-full bg-orange-100 flex items-center justify-center border-4 border-orange-500 shadow-lg">
                        <User className="w-10 h-10 text-orange-600" />
                      </div>
                    )}
                    <p className="text-sm font-semibold text-gray-700 mt-2">Host: {host.name}</p>
                ...
                {cook && (
                  <div className="flex flex-col items-center">
                    {cook.profileImage ? (
                      ...
                      <div className="w-20 h-20 rounded-full bg-amber-100 flex items-center justify-center border-4 border-amber-500 shadow-lg">
                        <User className="w-10 h-10 text-amber-600" />
                      </div>
                    )}
                    <p className="text-sm font-semibold text-gray-700 mt-2">Chef: {cook.name}</p>
              ...
              {/* Event Title */}
              <h3 className="text-2xl font-bold text-center mb-4 bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
                {event.title}
              </h3>

              {/* Cook's Story - Prominently Displayed */}
              {cook && (
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 border-2 border-orange-300 rounded-lg p-4 mb-4">
                  <p className="text-sm font-bold text-orange-600 mb-2">Why I'm Cooking This:</p>
                  <p className="text-sm italic text-gray-700 leading-relaxed">"{cook.story}"</p>
                </div>
              )}
              ...
              {hostImages && hostImages.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm font-semibold text-orange-600 mb-2">Event Space:</p>
              ...
              {/* Action Buttons */}
              <div className="flex gap-2 pt-4 border-t border-orange-200 mt-auto">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={(e) => {
                  ...
                  className="bg-white border-orange-300 hover:bg-orange-50 hover:border-orange-500 text-gray-700"
                >
                  <ImageIcon className="w-4 h-4 mr-2" />
                  All Photos ({hostImages?.length || 0})
                </Button>
                <Button
                  size="sm"
                  onClick={(e) => {
                ...
                >
                  <Heart className={`w-4 h-4 ${isSaved ? "fill-current text-red-500" : "text-gray-600"}`} />
                </Button>
              </div>
            </div>
            <div className="img-bg absolute inset-0 pointer-events-none" />
