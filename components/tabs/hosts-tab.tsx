...
export function HostsTab() {
  ...
  useEffect(() => {
    ...
    const demoHost = data.hosts.find((h) => h.id === DEMO_HOST_ID)
    setHost(demoHost || null)

    setCooks(data.cooks)

    // Get events hosted by demo host
    const hostEvents = data.events.filter((e) => e.hostId === DEMO_HOST_ID)
    setMyEvents(hostEvents)

    // Get seat requests for host's events
    const eventIds = hostEvents.map((e) => e.id)
    const requests = data.seatRequests.filter((r) => eventIds.includes(r.eventId))
    setSeatRequests(requests)

    // Get collaboration requests for this host
    const collabs = data.collaborationRequests.filter((r) => r.toHostId === DEMO_HOST_ID)
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <GlassCard className="p-6 bg-white/80 backdrop-blur-md">
          ...
          <div className="space-y-3">
            <h3 className="text-xl font-semibold text-gray-800">{host.spaceTitle}</h3>
            <p className="text-gray-600">{host.spaceDesc}</p>
                  ...
                  <div key={event.id} className="p-4 border-2 border-orange-200 rounded-lg space-y-2 bg-white/60">
                    ...
                    {seatRequests.filter((r) => r.eventId === event.id && r.status === "pending").length > 0 && (
                      <Badge variant="secondary" className="bg-orange-100 text-orange-700">
                        {seatRequests.filter((r) => r.eventId === event.id && r.status === "pending").length} pending
                        requests
                      </Badge>
                    )}
                  </div>
      ...
      <HostEventWizard open={showWizard} onOpenChange={setShowWizard} hostId={DEMO_HOST_ID} onSuccess={refreshData} />
